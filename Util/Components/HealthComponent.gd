	...
	for effect in damage_data.effects:
		if effect != null:
			...
			# 兼容旧的特效属性
				...
				var direction = (owner_body.global_position - attacker_position).normalized()
				owner_body.velocity = direction * effect.knockback_force
			elif "launch_force" in effect:
				owner_body.velocity.y = -effect.launch_force
