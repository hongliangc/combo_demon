class_name ForestSnail extends AgentBase

## ForestSnail — 缓慢地面敌人, AgentBase + AIController + SkillSet
## Idle ↔ Wander ↔ Chase → Dispatcher → GenericAttack; 无特殊技, 有重力
## 朝向由 AgentBase._update_facing() 处理 (无方向动画, 直接 flip_h)
...
func _setup_blackboard() -> void:
	...
	if bb:
		...
		bb.set_var(&"wander_speed", wander_speed)
