class_name Cyclope extends AgentBase

## Cyclope 敌人 — AgentBase + AIController + SkillSet 架构
## 行为: Idle ↔ Chase → Dispatcher → CyclopeAttack; Hit / Death 标准处理
## 无 Wander。CyclopeAttack 普通攻击; 20% 概率先蓄力再释放强化攻击。
## 注意: Cyclope(敌人) ≠ Cyclops(boss)。
