...
func physics_process_state(_delta: float) -> void:
	...
	# 计算切向（绕圈方向）
	var tangent := to_player.rotated(PI / 2.0 * _boss.circle_direction).normalized()
	...
	# 组合切向和径向运动
	_boss.velocity = (tangent + radial * distance_factor).normalized() * _boss.move_speed * circle_speed_multiplier

	# 随机改变绕圈方向
	if randf() < 0.01:
		_boss.circle_direction *= -1
