class_name Skull extends AgentBase

## Skull 敌人 — AgentBase + AIController + SkillSet 架构
## 行为: Idle ↔ Chase → Dispatcher → GenericAttack (/ SkullFrost); Hit / Death 标准处理
## 普通 Skull 仅 skull_strike 近战。SkullFrost 变体 (SkullFrost.tscn) 额外带寒霜新星。
...
## 寒霜新星 20% 概率门 —— skull_frost.tres 的 precondition_method
## (旧 SkullBlueFrostState.skill_probability = 0.2 的 faithful 等价)
func _can_skull_frost() -> bool:
	return randf() < 0.2
