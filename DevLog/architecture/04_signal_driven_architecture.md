...
| 组件 | 职责 | 信号类型 |
|------|------|----------|
| **HitBoxComponent** | 攻击检测，碰撞时调用 `take_damage()` | 无（直接方法调用） |
| **HurtBoxComponent** | 受击区域，转发伤害信号 | 发出 `damaged` |
| **HealthComponent** | 统一伤害处理：扣血、血条、特效、伤害数字 | 监听 HurtBoxComponent + 发出 `damaged` |
...
func _on_hitbox_area_entered_(area: Area2D):
    ...
    if area is HurtBoxComponent:
        # 关键：调用 HurtBoxComponent 的 take_damage 方法
        area.take_damage(damage, attacker_position)
```

#### 2. HurtBoxComponent 发出信号
...
func _ready() -> void:
    ...
    if hurtbox:
        # 关键：连接 HurtBoxComponent 的 damaged 信号
...
## 🎯 HitBoxComponent/HurtBoxComponent 碰撞检测架构

### 概述

HitBoxComponent/HurtBoxComponent 是战斗系统的基础碰撞检测组件，采用**方法调用 + 信号发射**的混合模式：

- **HitBoxComponent**: 攻击区域（子弹、武器），检测到碰撞后**调用方法**
- **HurtBoxComponent**: 受击区域（角色身体），被调用后**发出信号**
...
│                                                                             │
│   ┌───────────────┐                       ┌───────────────┐                │
│   │   HitBoxComponent      │   Godot 物理引擎       │   HurtBoxComponent     │                │
│   │  (Area2D)     │◄─────────────────────►│   (Area2D)    │                │
...
│           ↓                                                                 │
│   ┌─────────────────────────────────────────────────────────────────────┐  │
│   │  HitBoxComponent._on_hitbox_area_entered_(area)                              │  │
│   │  ├─ 1. 检查忽略碰撞组 (ignore_collision_groups)                     │  │
│   │  ├─ 2. update_attack() - 生成随机伤害值                             │  │
│   │  ├─ 3. if area is HurtBoxComponent:                                          │  │
...
│  玩家攻击敌人:                                                  │
│  ┌──────────────────┐         ┌──────────────────┐             │
│  │ Player HitBoxComponent    │         │ Enemy HurtBoxComponent    │             │
...
│  敌人攻击玩家:                                                  │
│  ┌──────────────────┐         ┌──────────────────┐             │
│  │ Enemy HitBoxComponent     │         │ Player HurtBoxComponent   │             │
│  │ collision_mask=4 │ ──────► │ collision_layer=4│             │
│  │ (检测第3层)       │         │ (在第3层)         │             │
│  └──────────────────┘         └──────────────────┘             │
│                                                                 │
│  碰撞层定义:                                                    │
│  Layer 3 (4)  = Player HurtBoxComponent                                 │
...
func _on_hitbox_area_entered_(area: Area2D):
    ...
    # 3. 【关键】调用 HurtBoxComponent 的方法（非信号）
    if area is HurtBoxComponent:
...
func _setup_signals() -> void:
    ...
    if hurtbox and health_component:
        # 【关键】HurtBoxComponent.damaged → HealthComponent.take_damage
...
| 环节 | 通信方式 | 原因 |
|------|----------|------|
| HitBoxComponent → HurtBoxComponent | **方法调用** | 同步、立即执行，确保伤害数据传递 |
| HurtBoxComponent → HealthComponent | **信号** | 解耦，HurtBoxComponent 不关心谁处理伤害 |
...
func _setup_signals() -> void:
    # 1. 连接 HurtBoxComponent → HealthComponent
    hurtbox.damaged.connect(health_component.take_damage)
