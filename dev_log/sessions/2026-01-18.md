...
- [x] 修复 GatherEffect 聚集范围受限于 Hitbox 的架构缺陷
...
1. **双重伤害问题**
   - 第一次伤害：Hitbox 碰撞触发（动画 0.4329 秒时）
...
- Hitbox 碰撞时使用的 SpecialAttack.tres 中的 GatherEffect 已经有了正确的目标位置
...
```gdscript
## 特殊攻击：在动画的攻击帧中调用
## 注意：伤害已经由 Hitbox 碰撞处理，此方法仅作为动画回调占位
func perform_special_attack() -> void:
	# Hitbox 碰撞已经应用了 SpecialAttack.tres 中的伤害和特效
	# 不需要在这里重复应用，避免双重伤害
	print("[特殊攻击] perform_special_attack 被调用（伤害由 Hitbox 处理）")
```

**关键点**：
- 避免双重伤害和特效应用
- 简化代码逻辑，所有伤害由 Hitbox 统一处理
- 保留方法作为动画回调占位符
...
用户指出了一个关键的架构缺陷：GatherEffect 通过 Damage 系统传递，导致聚集范围受限于 Hitbox 碰撞范围。

**错误架构**：
```
检测范围：300px 扇形 → 检测到 5 个敌人
Hitbox 范围：100px → 只碰撞到 2 个敌人
...
4. atk_sp 动画时间轴（优化后）：
   ...
   │
   └─ 2.9s: 调用 switch_to_physical()，禁用 Hitbox
...
**现象**：
- 第一次：Hitbox 碰撞触发（0.4329s），聚集到 (0.0, 0.0)，眩晕 1.0s
...
### 问题 4: GatherEffect 聚集范围受限于 Hitbox 碰撞范围（架构缺陷）
...
错误架构：
1. 检测范围：300px 扇形
2. Hitbox 范围：可能只有 100px
3. GatherEffect 通过 Damage 系统传递
4. 只有 Hitbox 碰撞到的敌人才会收到 Damage
...
### 2. 避免双重系统
- 项目中同时存在两套伤害应用机制（Hitbox + 手动调用）导致混乱
- **统一到一个系统**更易维护和调试
- 历史代码要及时清理，而非注释保留
